...
> 📁 **源文件**：`utils/agentcore_helper.py` → `create_agentcore_gateway` 函数
> 📁 **部署入口**：`deploy.py` → `create_gateway` 方法
...
> 📁 **源文件**：`lambda/api_spec.json`（LambdaTools Target）
...
> 📁 **源文件**：`lambda/api_spec_coupon.json`
...
> 📁 **部署逻辑**：`utils/agentcore_helper.py` → `create_coupon_lambda` 函数
> 📁 **部署入口**：`deploy.py` → `deploy_coupon_lambda` 方法
...
在 Agent 代码中，通过 MCP Client 连接 Gateway：
